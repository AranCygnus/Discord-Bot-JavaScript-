...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const replyMessage = `❌ 妳已經投過5️⃣了。`;


        if (data.Option === 5) {

            if (data.poll5Members.includes(interaction.user.id)) return interaction.reply({ content: replyMessage, ephemeral: true });
            ...
            const newEmbed = EmbedBuilder.from(message.embeds[0]).setFields(
                ...
                { name: `${emojiNumber[4]} ${data.poll5Value}`, value: `> ** ${data.poll5Vote + 1} **` },
            );
            ...
            data.poll5Members.push(interaction.user.id);
            data.save();
