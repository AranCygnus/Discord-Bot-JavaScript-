...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const replyMessage = `❌ 妳已經投過1️⃣了`;


        if (data.Option === 2) {

            if (data.poll1Members.includes(interaction.user.id)) return interaction.reply({ content: replyMessage, ephemeral: true });

            if (data.poll2Members.includes(interaction.user.id)) {
                data.poll2Vote = data.poll2Vote - 1
            };

            const newEmbed = EmbedBuilder.from(message.embeds[0]).setFields(
                { name: `${emojiNumber[0]} ${data.poll1Value}`, value: `> ** ${data.poll1Vote + 1} **` },
            ...
            data.poll1Members.push(interaction.user.id);
            ...
            const newEmbed = EmbedBuilder.from(message.embeds[0]).setFields(
                ...
                { name: `${emojiNumber[1]} ${data.poll2Value}`, value: `> ** ${data.poll2Vote} **` },
                { name: `${emojiNumber[2]} ${data.poll3Value}`, value: `> ** ${data.poll3Vote} **` },
                { name: `${emojiNumber[3]} ${data.poll5Value}`, value: `> ** ${data.poll5Vote} **` },
            );
